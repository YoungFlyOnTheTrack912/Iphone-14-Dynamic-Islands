class ProxyExtensions {
  ...
  // Performance: Async load scripts and styles
  ...
  // Performance: Add DNS prefetch for faster external requests
  static addPrefetching(html) {
    const prefetches = `<link rel="dns-prefetch" href="//cdn.jsdelivr.net"><link rel="preconnect" href="https://fonts.googleapis.com">`;
    return html.replace(/<head[^>]*>/i, (match) => match + prefetches);
  }

  // Performance: Remove render-blocking resources
  ...
  // Performance: Simplify DOM by removing hidden elements
  ...
  static apply(html, extensions = []) {
    ...
    extensions.forEach(ext => {
      ...
      if (ext === 'async') result = this.asyncLoadResources(result);
      if (ext === 'prefetch') result = this.addPrefetching(result);
      if (ext === 'noblockers') result = this.removeRenderBlockers(result);
      if (ext === 'simpledom') result = this.simplifyDOM(result);
    });
    return result;
  }
}
